...
namespace ExcelLoadTestReport.PageTemplates
{
    ...
    class PivotTableAndChartTemplate : ITemplateBase
    {
        ...
        public bool Fill(List<int> TestNumber, Dictionary<int, DAO.Counters> Counters,
            bool CreateChart = true, bool CreateRawSheets = true, string ChartName = "")
        {
            CreatePivotTable(TestNumber, Counters, ChartName, CreateChart, CreateRawSheets);
            return true;
        }
        ...
        private void CreatePivotTable(List<int> TestNumber, Dictionary<int, DAO.Counters> Counters,
            string ChartName, bool CreateChart = true, bool CreateFromRange = false)
        {
            ...
            if (!CreateFromRange)
            {
                ...
                //Globals.ThisAddIn.Application.ActiveWorkbook.Sheets.Add(After: Globals.ThisAddIn.Application.ActiveWorkbook.Sheets[Globals.ThisAddIn.Application.ActiveWorkbook.Sheets.Count]);
                //Globals.ThisAddIn.Application.ActiveSheet.Name = PvtSheetName;
                ...
                //pvtCache.CommandText = new string[] { String.Format("[dbo].[Pbx_GetSamplesForTest] {0}, N'{1}', '{2}'", TestNumber, Category, !string.IsNullOrEmpty(Counter) ? String.Format("N'{0}'", Counter) : "NULL") };
                //pvtCache.MaintainConnection = true;
                //pvtCache.CreatePivotTable(TableDestination: Globals.ThisAddIn.Application.ActiveCell,
                //TableName: PivotTableName,
                //DefaultVersion: Excel.XlPivotTableVersionList.xlPivotTableVersion14);
            }
            else
                ...
                using (var context = new Models.LoadTest2010Entities(_connString))
                {
                    ...
                    for (int i = 0; i < TestNumber.Count; i++)
                    {
                        foreach (var counterList in Counters)
                        {
                            ...
                            var conCatresult = (from items in context.PublixGetSamplesForTest(testNum, counterList.Value.CounterCategory, counterList.Value.CounterName, counterList.Value.CounterInstance, counterList.Value.FilterOutLoadTestRig) select items);
                            if (result == null)
                            {
                                result = conCatresult;
                            }
